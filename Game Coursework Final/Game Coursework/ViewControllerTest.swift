    ...
    self.collisionBehavior.addItem(AnimatedBirds)
            ...
            self.collisionBehavior.action =
            {
                for (oneSubView) in self.view.subviews{
                    if(oneSubView != self.movingroadImage && oneSubView != self.SequenceOfTreeImages) {

             if(self.aeroplaneImage.frame.intersects(AnimatedBirds.frame))
             {

                self.CoinsCollectedTotalScore = self.CoinsCollectedTotalScore - 5// remove 5 points from total score if plane collides with birds
...
@IBAction func PlayAgain(_ sender: UIButton) {
    ...
    self.view.bringSubviewToFront(aeroplaneImage)//bring the aeroplane images to the front
